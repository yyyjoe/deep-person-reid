...
class HACNN_MACNN(nn.Module):
    ...
    def __init__(self, num_classes, loss={'xent'}, nchannels=[128, 256, 384], feat_dim=512, learn_region=True, use_gpu=True, **kwargs):
        super(HACNN_MACNN, self).__init__()
        self.loss = loss
    ...
    def forward(self, x):
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x4_trans_i = self.stn(x3_out, x4_theta_i)
                x4_trans_i = F.upsample(x4_trans_i, (6, 7), mode='bilinear', align_corners=True)
                x4_local_i = x4_trans_i + x3_local_list[region_idx]
                x4_local_i = self.local_conv4(x4_local_i)
        ...
        if self.learn_region:
            x_local_list = []
            for region_idx in range(4):
                x_local_i = x4_local_list[region_idx]
